...
import { LANGUAGES, HOBBIES, SOFTWARE, DRIVING } from "@/lib/content";
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <Reveal y={28} className="md:col-span-2">
            <div className="h-full rounded-3xl border border-line bg-paper/[0.02] p-7 backdrop-blur-sm">
              <div className="mb-6 flex items-center gap-2.5 text-paper-dim">
                <Icon name="code" className="h-[18px] w-[18px] text-accent" />
                <span className="text-[0.72rem] uppercase tracking-[0.24em]">
                  Software
                </span>
              </div>
              <div className="flex flex-wrap gap-2.5">
                {SOFTWARE.map((s, i) => (
                  <motion.span
                    ...
                    whileHover={reduce ? {} : { y: -4 }}
                    data-cursor="hover"
                    className="inline-flex items-center rounded-full border border-line bg-ink px-4 py-2.5 text-[0.98rem] text-paper transition-colors duration-300 hover:border-accent/40 hover:text-accent"
                  >
                    {s}
                  </motion.span>
                ))}
          ...
          {/* Řidičské oprávnění */}
          <Reveal y={28} delay={0.08} className="md:col-span-1">
            <div className="h-full rounded-3xl border border-line bg-paper/[0.02] p-7 backdrop-blur-sm">
              <div className="mb-6 flex items-center gap-2.5 text-paper-dim">
                <Icon name="car" className="h-[18px] w-[18px] text-accent" />
                <span className="text-[0.72rem] uppercase tracking-[0.24em]">
                  Řidičské oprávnění
                </span>
              </div>
              <div className="flex flex-wrap gap-2.5">
                {DRIVING.map((d) => (
                  <span
                    key={d}
                    className="inline-flex items-center rounded-full border border-line bg-ink px-4 py-2.5 text-[0.98rem] text-paper"
                  >
                    {d}
                  </span>
                ))}
